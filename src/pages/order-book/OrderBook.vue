...
function filterData(newData, oldData) {
  let arr = newData.reduce((acc, [p, q]) => {
  ...
  if (oldData) {
    arr = [...arr, ...oldData].slice(0, limit.value);
  }
  return arr;
}
...
function updateShot() {
  ...
  if (lastU.value) {
    ...
    for (const [key, value] of Object.entries(bufferValue)) {
      if (key > lastUpdateId.value && lastUpdateId.value < value.u) {
